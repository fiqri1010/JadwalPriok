    ...
    >
      ...
      <div className="flex justify-between items-start gap-1 overflow-hidden w-full mb-0.5 min-w-0">
        ...
        <div className="flex items-center justify-end gap-0.5 flex-wrap overflow-hidden min-w-0">
          ...
          <button
            ...
            className={`hidden sm:block ${isWinamp ? 'rounded-none' : 'rounded'} p-0.5 transition-colors ${
              isExpanded
      ...
      {(liburNasional || holidayName || data.isManualHoliday) && (
        <div className="mb-0.5 min-w-0">
          <span
            className={`block w-full truncate ${isWinamp ? 'rounded-none bg-[#FF3366] text-white font-mono' : 'rounded text-white'} px-0.5 py-0.2 text-[5.5px] sm:text-[6.5px] lg:text-[7px] font-black leading-tight text-center shadow-2xs ${
              isWinamp ? 'bg-[#FF3366]' : liburNasional ? 'bg-[#FF3366]' : 'bg-rose-600'
            }`}
        ...
        >
          ...
          {!isTanggalMerah && !data.isMasuk && data.shift !== 'OFF' && (
            <div className="space-y-1">
              {/* Gunakan OFF Button */}
              <div className={`rounded-lg p-1 border transition-all ${
                isOffDisabled
            ...
            <div className="rounded-lg p-1 border border-amber-200 bg-amber-50/80 text-amber-900 space-y-1">
              ...
              <div className="grid grid-cols-2 gap-1">
                <button
                  ...
                  className={`rounded py-0.5 text-[8px] font-black transition-all cursor-pointer ${
          ...
          <div className="grid grid-cols-2 gap-1">
            ...
            <div
              className={`flex flex-col justify-center rounded-lg p-1 border ${
                canHaveST
            ...
            >
              <label
                className={`flex items-center justify-between ${
                  canHaveST ? 'cursor-pointer' : 'cursor-not-allowed pointer-events-none opacity-50'
                }`}
                title={canHaveST ? "ST CP (Surat Tugas Tambahan - Mendapatkan Cuti Pengganti +1)" : "ST CP hanya aktif di hari libur (Weekend / Tanggal Merah) dan bukan shift OFF/CUTI"}
              >
