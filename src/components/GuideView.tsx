...
export const GuideView: React.FC<GuideViewProps> = ({ theme = 'default' }) => {
    ...
    const sections: GuideSection[] = [
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div className={neutralSubCardClass}>
                            <h5 className={`font-bold text-xs mb-2 ${isDark ? 'text-indigo-400' : isWinamp ? 'text-[#00FF00]' : 'text-indigo-700'}`}>
                                A. Shift Pagi / Normal (Graha, TPSL, NPCT, PM, G, L, N)
                            </h5>
        ...
        {
            id: 'roadmap',
            title: '10. Fitur Sedang Dikembangkan',
            badge: 'Roadmap & Future',
            badgeColor: isWinamp
            ...
            content: (
                <div className="space-y-4">
                    <p className={`text-xs sm:text-sm leading-relaxed ${isWinamp
                        ...
                        Berikut adalah modul dan peningkatan fungsionalitas yang sedang aktif dalam tahap riset dan pengembangan untuk pembaruan sistem berikutnya:
                    </p>
                    <div className={`divide-y ${isWinamp
                            ? 'divide-zinc-800'
                            : isDark
                                ? 'divide-white/10'
                                : isVista
                        ...
                        {[
                            {
                                no: '1',
                                title: 'Dukungan cuti setengah hari',
                                desc: 'Pencatatan dan kalkulasi jam kerja untuk cuti setengah hari (pagi atau siang) dengan penyesuaian jam dinas dan absensi proporsional.',
                            },
                            {
                                no: '2',
                                title: 'Dukungan ST selain ST Cuti Pengganti',
                                desc: 'Akomodasi ragam Surat Tugas (ST) kedinasan lainnya, seperti ST BKO, ST Pengawasan Khusus, Pelatihan/Diklat, maupun penugasan resmi non-piket.',
                            },
                            {
                                no: '3',
                                title: 'Fitur Rule Set untuk mengatur kriteria performa',
                                desc: 'Konfigurasi fleksibel parameter batas jam masuk/pulang, ambang batas toleransi keterlambatan CEISA, serta skema pembobotan skala performa.',
                            },
                            {
                                no: '4',
                                title: 'Tukar Piket dengan pegawai lain',
                                desc: 'Fasilitas pertukaran jadwal piket antar pegawai secara resmi dengan pencatatan riwayat persetujuan dan rekapitulasi otomatis.',
                            },
                            {
                                no: '5',
                                title: 'Ambil/Diambil Piket dari/oleh pegawai lain',
                                desc: 'Fleksibilitas pelimpahan tugas piket atau pengambilan jadwal piket tambahan antar rekan kerja secara transparan dan terdata.',
                            },
                            {
                                no: '6',
                                title: 'Lainnya',
                                desc: 'Peningkatan fitur yang telah ada, penambahan fitur simpan ST, serta ide-ide lainnya.',
                            },
                        ].map((item) => (
                            <div key={item.no} className="py-3 first:pt-0 last:pb-0 flex items-start gap-3">
                                <span
                                    className={`flex h-6 w-6 shrink-0 items-center justify-center text-xs font-black ${isWinamp
                                            ? 'rounded-none bg-zinc-800 text-[#00FF00] border border-[#00FF00]/40 font-mono'
                                            : isDark
                                                ? 'rounded-full bg-purple-500/20 text-purple-300 ring-1 ring-purple-500/30'
                                                : isVista
                                                    ? 'rounded-full bg-purple-600 text-white shadow-xs'
                                                    : 'rounded-full bg-purple-600 text-white shadow-xs'
                                        }`}
                                >
                                    {item.no}
                                ...
                                <div className="flex-1 min-w-0">
                                    <h4
                                        className={`text-xs sm:text-sm font-bold ${isWinamp
                                    ...
                                    <p
                                        className={`text-xs sm:text-sm mt-0.5 leading-relaxed ${isWinamp
                                                ? 'text-zinc-300 font-mono'
                                                : isDark
                                                    ? 'text-slate-300'
                                                    : isVista
