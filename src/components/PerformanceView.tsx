...
export const PerformanceView: React.FC<PerformanceViewProps> = ({
  ...
  const handleExportPDF = () => {
    try {
      ...
      doc.save(`Laporan_Performance_CEISA_${periodType}_${selectedYear}.pdf`);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className={`flex items-center space-x-1.5 rounded-xl border px-3 py-2 text-xs font-bold transition-all shadow-2xs cursor-pointer ${
                isWinamp
        ...
        <div className={`mt-4 pt-4 border-t flex flex-col lg:flex-row lg:items-center justify-between gap-3 ${isDark ? 'border-[#2A2A2A]' : isVista ? 'border-white/30' : isWinamp ? 'border-[#333333]' : 'border-slate-100'}`}>
          ...
          <div className={`flex items-center p-1 rounded-xl overflow-x-auto ${tabListClass} ${isWinamp ? 'rounded-none' : ''}`}>
            <button
              ...
              className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all shrink-0 cursor-pointer ${isWinamp ? 'rounded-none' : ''} ${
                periodType === 'month' ? tabActiveClass : tabInactiveClass
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all shrink-0 cursor-pointer ${isWinamp ? 'rounded-none' : ''} ${
                periodType === 'quarter' ? tabActiveClass : tabInactiveClass
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all shrink-0 cursor-pointer ${isWinamp ? 'rounded-none' : ''} ${
                periodType === 'semester' ? tabActiveClass : tabInactiveClass
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all shrink-0 cursor-pointer ${isWinamp ? 'rounded-none' : ''} ${
                periodType === 'year' ? tabActiveClass : tabInactiveClass
              }`}
            >
              ...
              <div className={`flex items-center space-x-1 p-1 rounded-xl border ${selectorBoxClass} ${isWinamp ? 'rounded-none' : ''}`}>
                {[1, 2, 3, 4].map((q) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-xs font-black transition-all cursor-pointer ${isWinamp ? 'rounded-none' : ''} ${
                      selectedQuarter === q
            ...
            {periodType === 'semester' && (
              <div className={`flex items-center space-x-1 p-1 rounded-xl border ${selectorBoxClass} ${isWinamp ? 'rounded-none' : ''}`}>
                <button
                  ...
                  className={`px-3 py-1 rounded-lg text-xs font-black transition-all cursor-pointer ${isWinamp ? 'rounded-none' : ''} ${
                    selectedSemester === 1
                      ? isWinamp ? 'bg-[#00FF00] text-black font-black' : 'bg-indigo-600 text-white shadow-2xs'
                      : isDark ? 'text-slate-400 hover:bg-[#333333]' : isWinamp ? 'text-[#00FF00] hover:bg-zinc-900' : 'text-slate-600 hover:bg-slate-200/60'
                  }`}
          ...
          <div className="text-xs space-y-1 w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2 pt-1 text-[11px]">
              <div className={`rounded-xl p-2.5 border ${rulesInnerBoxClass} ${isWinamp ? 'rounded-none font-mono' : ''}`}>
                <span className={`font-black block mb-1 ${isDark ? 'text-teal-300' : isWinamp ? 'text-[#00FF00]' : 'text-teal-800'}`}>Shift G, L, N, PM, SM:</span>
              ...
              <div className={`rounded-xl p-2.5 border ${rulesInnerBoxClass} ${isWinamp ? 'rounded-none font-mono' : ''}`}>
                <span className={`font-black block mb-1 ${isDark ? 'text-blue-300' : isWinamp ? 'text-[#00E5FF]' : 'text-blue-800'}`}>Shift M (Malam):</span>
                <ul className="space-y-0.5">
              ...
              <div className={`rounded-xl p-2.5 border ${rulesInnerBoxClass} ${isWinamp ? 'rounded-none font-mono' : ''}`}>
                <span className={`font-black block mb-1 ${isDark ? 'text-rose-300' : isWinamp ? 'text-[#FF3366]' : 'text-rose-800'}`}>Hold Dokumen (Pengecualian):</span>
                <ul className="space-y-0.5">
                  <li>• Jika status <strong className={isDark ? 'text-rose-300' : isWinamp ? 'text-[#FF3366]' : 'text-rose-700'}>Hold Dokumen aktif</strong>, hari tersebut <strong className={isDark ? 'text-rose-300' : isWinamp ? 'text-[#FF3366]' : 'text-rose-700'}>TIDAK DIHITUNG</strong> dalam penilaian & pembagi rata-rata.</li>
                </ul>
              </div>
            </div>
          </div>
      ...
      <div className={`rounded-2xl p-4 sm:p-5 border shadow-xs space-y-3 ${quarterOuterClass} ${isWinamp ? 'rounded-none font-mono' : ''}`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2">
          {[
            ...
            <div
              key={item.label}
              className={`rounded-xl p-3 border text-center transition-all ${isWinamp ? 'rounded-none font-mono' : ''} ${
                idx === 6
          ...
          <div className="flex items-center gap-2 self-start sm:self-auto">
            {sortConfig && (
              <button
                ...
                className={`flex items-center space-x-1 px-2.5 py-1 text-[11px] font-bold transition-colors cursor-pointer ${
                  isWinamp
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-2 px-3 text-center">
                      <span className={`inline-block px-2 py-0.5 rounded font-black text-[10px] font-mono ${
                        isWinamp
                    ...
                    <td className={`py-2 px-3 text-center font-mono font-bold whitespace-nowrap ${
                      isDark ? 'text-slate-100' : isWinamp ? 'text-[#00FF00]' : 'text-slate-900'
                    }`}>
                      {row.absenCeisa || (
                    ...
                    <td className="py-2 px-3 text-center whitespace-nowrap">
                      {row.isHoldDokumen ? (
                        ...
                        <span
                          className={`inline-flex items-center justify-center h-6 w-6 rounded-full font-mono text-xs font-black shadow-2xs ${
                            isWinamp ? 'rounded-none border' : ''
                          } ${
                    ...
                    <td className={`py-2 px-3 text-[11px] max-w-xs truncate ${
                      isWinamp ? 'text-[#00FF00]/50' : isDark ? 'text-slate-400' : 'text-slate-500'
                    }`}>
                      {row.note || '-'}
                    </td>
                  </tr>
