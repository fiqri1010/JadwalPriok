...
export function calculateCeisaScore(
  ...
  // 5. Shift Graha, NPCT, TPSL, SM, PM (G, L, N, PM, SM):
  ...
  if (['Graha', 'NPCT', 'TPSL', 'SM', 'PM', 'G', 'L', 'N'].includes(shift)) {
...
export function calculateDayResult(
  ...
  // "Jadwal selain OFF dan CUTI jika ST CP dicentang di hari libur (weekend/tanggal merah), maka mendapatkan CUTI PENGGANTI +1 otomatis."
  if (isTanggalMerah && shift !== 'OFF' && shift !== 'CUTI' && data.isSuratTugasTambahan) {
